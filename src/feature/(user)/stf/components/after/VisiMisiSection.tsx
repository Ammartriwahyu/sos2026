// VisiMisiSection.tsx

"use client";
import React from "react";
import CaketangCard from "./CaketangCard";
import { Caketang } from "@/api/services/user/stf";
import Image from "next/image";
import { formatText } from "@/lib/utils";

interface VisiMisiSectionProps {
  caketangList: Caketang[];
  isLoading: boolean;
  error: string | null;
  activeCardId: string | null;
  setActiveCardId: (id: string) => void;
}

const VisiMisiSection = ({
  caketangList,
  activeCardId,
  setActiveCardId,
}: VisiMisiSectionProps) => {
  const activeCaketang = caketangList?.find(
    (caketang: Caketang) => caketang.id_caketang === activeCardId,
  );

  return (
    <div className="mx-auto flex w-full flex-col items-center justify-center py-16 md:py-24 px-4 gap-20 md:gap-32 text-white lg:px-8 md:px-12 xl:px-24">
      {/* Cards Row */}
      <div className="flex w-full flex-nowrap justify-center gap-4 md:gap-12 lg:gap-20">
        {caketangList?.map((caketang: Caketang) => (
          <CaketangCard
            key={caketang.id_caketang}
            data={caketang}
            isActive={caketang.id_caketang === activeCardId}
            onClick={() => setActiveCardId(caketang.id_caketang)}
          />
        ))}
      </div>

      {/* Detail Section */}
      {activeCaketang && (
        <div className="grid grid-cols-1 lg:grid-cols-12 w-full gap-10 md:gap-16 items-start">
          {/* Left: Photo */}
          <div className="col-span-1 lg:col-span-4 flex flex-col justify-center items-center gap-4">
            <div className="flex flex-col w-full max-w-sm rounded-[4rem] rounded-b-xl overflow-hidden shadow-2xl bg-[#C4BCEB]">
              <div className="w-full pt-2 px-2 pb-2">
                <Image
                  src={activeCaketang.foto}
                  width={400}
                  height={400}
                  alt="Foto Caketang"
                  className="w-full h-80 md:h-96 lg:h-[26rem] object-cover rounded-t-[3.5rem] rounded-b-lg"
                />
              </div>
            </div>

            {/* Separate Name Container */}
            <div className="flex flex-col w-full max-w-sm rounded-[2rem] shadow-xl bg-[#B084D1] justify-center items-center px-4 py-6 text-center">
              <p className="text-white font-bold text-xl md:text-2xl uppercase leading-tight">
                {activeCaketang.nama}
              </p>
            </div>
          </div>

          {/* Right: Details (Prodi, Visi, Misi) */}
          <div className="col-span-1 lg:col-span-8 flex flex-col gap-10">
            <h4 className="text-white text-3xl md:text-4xl lg:text-5xl font-bold text-center lg:text-left leading-tight">
              {activeCaketang.prodi}
            </h4>

            <div className="flex flex-col gap-4">
              <h5 className="text-xl md:text-2xl font-bold text-white/90">
                Visi :
              </h5>
              {activeCaketang.visi &&
                formatText(activeCaketang.visi).map((line, index) => (
                  <p
                    key={index}
                    className="text-lg md:text-xl leading-8 font-normal text-justify text-white/80"
                  >
                    {line}
                  </p>
                ))}
            </div>

            <div className="flex flex-col gap-4">
              <h5 className="text-xl md:text-2xl font-bold text-white/90">
                Misi :
              </h5>
              <div className="flex flex-col gap-2">
                {activeCaketang.misi &&
                  formatText(activeCaketang.misi).map((line, index) => (
                    <p
                      key={index}
                      className="text-lg md:text-xl leading-8 font-normal text-justify text-white/80"
                    >
                      {line}
                    </p>
                  ))}
              </div>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};

export default VisiMisiSection;
